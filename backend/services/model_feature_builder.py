...
def apply_dti_risk_floor(
    ...
    *,
    ...
    # [Lớp 1 — Sàn DTI] DTI ≤ 40%: không hiệu chỉnh, mô hình tin tưởng hoàn toàn
    if dti_value <= 0.40:
        return probability

    # [Lớp 1 — Sàn DTI] Vùng cảnh báo 40–55%: sàn tăng tuyến tính từ low_threshold (0.2) → high_threshold (0.4)
    # Phản ánh nguyên tắc ngân hàng: gánh nặng nợ tăng dần → rủi ro phải tăng dần
    if dti_value <= 0.55:
        progress = (dti_value - 0.40) / 0.15
        floor = low_threshold + (high_threshold - low_threshold) * progress
    # [Lớp 1 — Sàn DTI] Vùng căng thẳng cao 55–70%: sàn tiếp tục nhích lên trên high_threshold (+0.05)
    elif dti_value <= 0.70:
        progress = (dti_value - 0.55) / 0.15
        floor = high_threshold + 0.05 * progress
    # [Lớp 1 — Sàn DTI] Vùng cực đoan > 70%: sàn cứng, có thể lên tới 0.65
    else:
        progress = min((dti_value - 0.70) / 0.30, 1.0)
        floor = high_threshold + 0.05 + 0.20 * progress

    # Kết quả = max(xác_suất_thô, sàn): nếu mô hình đã cho xác suất cao hơn thì giữ nguyên
