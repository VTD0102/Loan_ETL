...
Các tài nguyên đắt đỏ (kết nối LLM, embedding, vector store, reranker) được **cache/lazy initialize** để tránh khởi tạo lại ở mỗi request. Những thành phần nằm trên đường nóng và có khả năng bị nhiều request chạm cùng lúc như LCEL chain, retriever và intent classifier dùng mẫu **double-checked locking** (xem mục 5.7.3); một số helper như query-rewrite LLM, summary LLM và reranker hiện là lazy singleton đơn giản. Cụ thể:
...
> **Lưu ý kỹ thuật:** `_rewrite_llm`, `_summary_llm` và `_singleton` hiện chưa có khóa khởi tạo. Điều này không làm sai kết quả nghiệp vụ, nhưng trong tải đồng thời cao có thể gây khởi tạo lặp hiếm gặp; nếu đưa vào production nhiều worker/luồng, nên harden các singleton này bằng lock giống `get_chain()` và `get_retriever()`.
...
| `rag_loan_reasoner_enabled` | `True` | Bật bộ đề xuất mềm bằng LLM cho công cụ điều chỉnh khoản vay (mục 5.7.2) |
...
Các biến chính được bơm vào `SYSTEM_TEMPLATE` (`prompts.py`) gồm: tên khách hàng, hướng dẫn giọng điệu, hướng dẫn theo ý định, hồ sơ khách hàng (User Context), tóm tắt hội thoại, lịch sử gần đây qua `chat_history`, tài liệu liên quan (Knowledge Base) và câu hỏi hiện tại. Quy tắc số 7 trong system prompt nêu rõ thứ tự ưu tiên: *"Với câu hỏi cá nhân, LUÔN ưu tiên THÔNG TIN HỒ SƠ KHÁCH HÀNG. TÀI LIỆU LIÊN QUAN chỉ là bổ trợ chính sách."* Đây chính là cơ chế hòa giải khi hai nguồn tri thức có thể mâu thuẫn.
...
CLI ingest hỗ trợ ba chế độ: `--dry-run` (liệt kê tài liệu + số chunk, không ghi), mặc định (thêm tài liệu vào collection hiện có), và `--recreate` (xóa rồi dựng lại, phá hủy). Cần lưu ý rằng nhánh mặc định hiện gọi `store.add_documents(chunks)` mà **chưa truyền point ID ổn định**; vì vậy không nên coi đây là upsert idempotent tuyệt đối. Nếu chạy ingest lặp lại sau khi sửa tài liệu/chunking, cách an toàn nhất hiện tại là dùng `--recreate` để tránh nguy cơ nhân bản điểm dữ liệu trong Qdrant.

---

## 5.6 Giai đoạn Runtime: pipeline sáu bước

Mỗi lượt chat đi qua hàm `chain.invoke()` và được xử lý tuần tự qua sáu bước, trải dài từ khâu kiểm soát đầu vào cho tới khâu hậu kiểm đầu ra. Điều đáng nói không chỉ là trình tự sáu bước ấy, mà là **triết lý vận hành suy giảm duyên dáng** (degrade gracefully) cho các thành phần phụ trợ: viết lại câu hỏi, truy xuất tài liệu, xếp hạng lại và tóm tắt hội thoại đều có cơ chế fallback để chất lượng giảm dần thay vì làm đứt toàn bộ pipeline. Riêng lời gọi LLM chính là ngoại lệ quan trọng: nếu timeout hoặc lỗi kết nối xảy ra ở bước sinh câu trả lời, `chat_service` sẽ lưu một assistant message đánh dấu lỗi và trả HTTP 503 với thông báo an toàn. Vì vậy, mô tả chính xác là hệ thống **chịu lỗi tốt ở các lớp phụ trợ**, còn lỗi ở LLM chính vẫn được coi là sự cố dịch vụ cần xử lý ở tầng điều phối.
...
Ba kỹ thuật trình bày trong mục này là những gì nâng trợ lý CreditIntel vượt lên trên một chatbot hỏi-đáp thông thường: bộ nhớ hội thoại cho phép nó nhớ và nối mạch câu chuyện, công cụ điều chỉnh khoản vay cho phép nó hành động thay vì chỉ nói, và mẫu cache/singleton giúp nó phục vụ nhiều người cùng lúc một cách hiệu quả hơn.

### 5.7.1 Chat Memory: cửa sổ trượt kết hợp đệm tóm tắt lười

Một hội thoại càng kéo dài thì prompt càng phình to và càng tốn token, nhưng cắt cụt lịch sử lại khiến trợ lý quên mất khách hàng vừa nói gì. Hàm `load_memory` trong `memory.py` hóa giải mâu thuẫn này bằng cách kết hợp hai cơ chế bổ sung cho nhau. Cơ chế thứ nhất là cửa sổ trượt theo ngân sách token. Hệ thống lấy các tin nhắn cũ của phiên (bỏ qua các tin bị đánh dấu lỗi; trong `chat_service.send()` còn loại riêng tin user vừa lưu bằng `exclude_message_id`), rồi để hàm `_split_window` duyệt từ tin mới nhất ngược về quá khứ, cộng dồn chi phí token ước lượng thô theo công thức `len(text) // 4`, cho tới khi chạm trần ngân sách `rag_memory_window_token_budget` là 2000 token. Phần "gần đây" nằm trong cửa sổ ấy được đưa nguyên văn vào biến `chat_history` của prompt. Trong phạm vi lịch sử cũ được đưa vào `load_memory`, tin gần nhất không bị loại bỏ chỉ vì vượt ngân sách; còn câu hỏi hiện tại của khách hàng được truyền riêng qua biến `{question}`, không phụ thuộc vào `chat_history`.
...
Khi ý định đã được xác nhận, hàm `find_best_reapplication_option` dựng tập ứng viên từ **hai nguồn rồi hợp nhất**, thay vì thử tuần tự từng chiến lược rồi dừng sớm. Nguồn thứ nhất là một **bộ đề xuất mềm bằng LLM** (`loan_adjustment_reasoner`, bật/tắt qua cờ `rag_loan_reasoner_enabled`): từ một bản tóm tắt rủi ro tất định của đơn bị từ chối (DTI, nợ hàng tháng, thu nhập, nợ xấu, số khoản vay trước…), nó gọi `gemini-2.5-flash` ở `temperature=0` để đề xuất tối đa sáu phương án (số tiền, kỳ hạn, chiến lược, lý do) dưới dạng JSON, với ràng buộc chỉ được giảm số tiền và chỉ giữ nguyên hoặc kéo dài kỳ hạn; mọi lỗi gọi hàm hay JSON hỏng đều khiến nó trả về danh sách rỗng để lui an toàn về lưới cứng. Nguồn thứ hai là **lưới cứng tất định** (`_grid_candidates`): nhánh kéo dài kỳ hạn thử các mốc dài hơn trong tập {12, 24, 36, 48, 60} tháng, còn nhánh giảm số tiền cố định ở kỳ hạn sáu mươi tháng rồi thử hạn mức hệ thống đề xuất cùng các mốc 75%, 50%, 25% của số tiền ban đầu và mức sàn 500 đô-la. Hàm `merge_candidates` gộp hai nguồn (ưu tiên ứng viên LLM), làm sạch và khử trùng: loại các phương án tăng tiền, kỳ hạn ngoài tập cho phép hay ngắn hơn hiện tại, và bỏ luôn form trùng với đơn gốc đã bị từ chối.

Điểm mấu chốt về độ tin cậy là **mọi ứng viên — bất kể đến từ LLM hay lưới cứng — đều được đưa trở lại đúng mô hình ML production qua `ml_service.predict` trong một lượt quét duy nhất** (không còn dừng sớm theo từng stage), để lấy xác suất vỡ nợ thực chứ không phải một con số phỏng đoán. Một ứng viên chỉ được coi là đạt khi xác suất vỡ nợ nằm dưới ngưỡng `AUTO_REVIEW_THRESHOLD` là 0.4 và đồng thời vượt qua được bộ kiểm tra `validate_confirmed_values`. Các phương án đạt được xếp hạng bằng **một khoá thống nhất ưu tiên thay đổi ít nhất so với đơn gốc** (`_unified_rank`: tổng tỉ lệ giảm tiền cộng tăng kỳ hạn nhỏ nhất trước, rồi tới xác suất vỡ nợ thấp hơn, số tiền lớn hơn và kỳ hạn ngắn hơn), và công cụ trả về tối đa ba phương án hàng đầu; mỗi phương án mang theo nhãn chiến lược `extend_term`, `reduce_amount` hoặc `both` cùng một câu lý do (`rationale`) được đưa vào ngữ cảnh RAG. Trong trường hợp không ứng viên nào lọt xuống dưới ngưỡng, công cụ chuyển sang trạng thái `fallback_proposal`, trình bày ba biểu mẫu tốt nhất quan sát được (xác suất vỡ nợ thấp nhất) kèm cảnh báo minh bạch rằng chúng vẫn vượt ngưỡng an toàn và cần admin duyệt thủ công.

Các phương án tìm được sau đó được tiêm vào ngữ cảnh của LLM thông qua `_format_loan_adjustment_context`, hiện mở đầu bằng khối "Kết quả tool mô phỏng điều chỉnh khoản vay" kèm một chỉ dẫn quan trọng dặn LLM trình bày các phương án như sự thật đã được tính sẵn, đồng thời hướng dẫn khách nhắn "Đồng ý" hoặc "Xác nhận" để nộp, hay "Hủy" để bỏ. Quy tắc số 9 trong system prompt cấm tuyệt đối việc LLM giả vờ "đang chạy tính toán nền", bởi mọi phương án phải có sẵn ngay trong lượt hiện tại. Tuy nhiên, có một điểm cần chỉnh cho chặt: prompt hiện nhắc tới marker "PHƯƠNG ÁN ĐỀ XUẤT TỪ HỆ THỐNG", trong khi code đang dùng tên khối khác; nên thống nhất hai tên này để điều kiện trong prompt rõ ràng và dễ kiểm thử hơn. Toàn bộ thao tác nộp lại được bảo vệ bằng một vòng xác nhận có trạng thái: phương án được lưu vào `session.pending_action` với thời gian sống ba mươi phút theo hằng số `PENDING_ACTION_TTL_MINUTES`. Ở lượt kế tiếp, hàm `_handle_pending_loan_adjustment_response` đọc câu trả lời của khách; nếu là phủ định thì hủy bỏ, còn nếu là khẳng định thì `_confirm_pending_loan_adjustment` gọi `application_service.confirm` để tạo một đơn vay hoàn toàn mới, chỉ thay đổi số tiền và kỳ hạn, giữ nguyên mọi số liệu còn lại, rồi báo lại mã đơn mới cùng xác suất vỡ nợ tương ứng. Hồ sơ bị từ chối cũ tuyệt đối không bao giờ bị sửa đổi, bảo toàn tính toàn vẹn của lịch sử.

> **Quyết định thiết kế: vì sao mọi phương án phải qua đúng mô hình ML.** Một cách làm cẩu thả là để LLM tự đoán kỳ hạn nào sẽ được duyệt rồi hứa luôn với khách, nhưng đó chính là mảnh đất màu mỡ cho ảo giác và những lời hứa sai. Kiến trúc hiện tại cho LLM một vai trò có ích nhưng bị kiểm soát chặt: nó được phép **đề xuất** ứng viên (gợi ý giảm tiền hay kéo dài kỳ hạn theo yếu tố rủi ro chính), song mỗi đề xuất ấy, cùng toàn bộ lưới cứng, đều phải chạy qua chính mô hình đang vận hành trong production, và công cụ chỉ trình bày những phương án đã được kiểm chứng nằm dưới ngưỡng 0.4. Nói cách khác, LLM **đề xuất** nhưng mô hình ML mới là bên **quyết định** phương án nào đủ an toàn để hiện ra; quyền phán quyết tín dụng vẫn nằm trọn trong tay mô hình và bộ luật nghiệp vụ, còn quyết định phê duyệt cuối cùng thuộc về admin.

### 5.7.3 Singleton và an toàn đa luồng

Backend FastAPI phục vụ nhiều request đồng thời trên nhiều luồng, trong khi việc khởi tạo các client LLM, embedding hay Qdrant, và đặc biệt là nạp mô hình reranker nặng khoảng 1,1 GB, đều tốn kém. Để giảm chi phí khởi tạo, các tài nguyên dùng chung được cache ở mức module. Với các thành phần chính như chain, retriever và intent classifier, code dùng mẫu khóa kiểm tra hai lần (double-checked locking):
...
Kiểm tra ngoài khóa cho **đường nóng** (đã khởi tạo) chạy không tranh chấp; khóa chỉ chặn ở lần đầu, và kiểm tra lần hai bên trong khóa đảm bảo **đúng một** luồng khởi tạo cho các thành phần có lock. Mẫu này lặp lại ở `get_retriever`, `_get_classifier_llm`. Reranker đi xa hơn với **lazy loading**: `Reranker._ensure_loaded` chỉ thực sự nạp `TextCrossEncoder` ở lần `rerank` đầu tiên, nên nếu rerank bị tắt qua config, mô hình 1.1 GB không bao giờ được tải. Dù vậy, như đã ghi ở mục 5.3, một số singleton helper hiện chưa có lock; đây là điểm nên hoàn thiện nếu yêu cầu vận hành đặt nặng tính an toàn đa luồng tuyệt đối.
...
Bộ eval **31 case** được chạy lại trên **pipeline hiện hành** (hybrid + cross-encoder rerank `k=20→12`, `top_k=4` parent, LLM `gemini-2.5-flash` `temperature=0.3`), trên collection `creditintel-kb` với KB gồm 3 tài liệu. Dry-run ingest hiện tại cho **76 child chunk**; collection dùng cho eval cần bảo đảm đã được dựng lại bằng `--recreate` để tránh lệch số điểm do ingest mặc định chưa idempotent tuyệt đối. Không case nào lỗi gọi hàm. Kết quả tổng hợp:
...
Tách theo nhóm chủ đề cho thấy chất lượng phân bố rất không đều, đồng thời làm lộ rõ một **artifact của thước đo** và vài điểm cần chỉnh trong cách chấm:
...
**Đọc kết quả, điểm mấu chốt về nhóm `personalized`.** Năm case `personalized` "rớt" (`overall ≈ 0.50`) chủ yếu làm lộ một **artifact của thước đo**, nhưng không nên diễn giải đơn giản là "câu trả lời hoàn toàn đúng". Các câu hỏi cá nhân hóa lấy grounding chính từ **User Context**, bốn khối hồ sơ tính sẵn bằng luật cứng (mục 5.4.2), trong khi `Context Precision` hiện chỉ chấm độ sạch của tài liệu KB trả về. Dataset `personalized` đặt `expected_sources=[]` và `expected_context_terms=[]`; theo công thức hiện tại, nếu pipeline vẫn trả về tài liệu KB bổ trợ thì precision bị chấm 0. Vì vậy, điểm precision của nhóm này không phản ánh đúng chất lượng grounding bằng hồ sơ cá nhân.

Faithfulness trung bình của nhóm `personalized` vẫn khá cao (0.837), cho thấy phần lớn thông tin bắt buộc có xuất hiện và có cơ sở trong User Context. Tuy nhiên, hai case `PERSONAL-02` và `PERSONAL-04` bị thiếu `must_include` vì câu trả lời dùng định dạng số kiểu `8.000`/`10.000`, trong khi dataset chỉ chấp nhận các biến thể `8000`, `8,000`, `$8,000`. Đây là lỗi thiết kế metric/normalization hơn là lỗi nghiệp vụ rõ ràng, nhưng vẫn cần sửa để báo cáo đánh giá không tự mâu thuẫn. **Nếu tách riêng nhóm personalized khỏi phép đo precision của KB**, Context Precision trên 26 case còn lại đạt **≈ 0.92**, và số case phi-personalized đạt là **23/26 (~88%)**; ba case phi-personalized chưa đạt là `GUARDRAIL-05`, `EDGE-04`, `EDGE-01`.

> **Hệ quả cho khung đo.** Đây là hạn chế đã biết của metric hiện tại: nó đo *precision của retrieval KB* nên không phù hợp với các câu trả lời grounding-bằng-hồ-sơ. Hướng khắc phục (mục 5.9.3): tách một metric **"user-context grounding"** riêng cho nhóm personalized, đồng thời chuẩn hóa định dạng số tiếng Việt/US (`8.000`, `8,000`, `$8,000`) trước khi khớp `must_include`.
...
> **Kết luận eval.** Thay đổi code ngày 02/06 (`chain.py`/`retriever.py`) **không gây hồi quy ở mức pipeline** theo ngưỡng run-level hiện tại (Δ overall temp=0 chỉ −0.007). Pipeline hiện hành đạt **overall ≈ 0.82**, mạnh ở `policy`/`faq` (~0.95), trung bình ở `guardrail`/`edge_case`, và nhóm `personalized` đang bị metric precision của KB chấm không phù hợp. Một regression case lẻ (`EDGE-01`) do thay đổi thứ tự retrieval cần rà lại; ngoài ra `GUARDRAIL-05` và `EDGE-04` là hai case phi-personalized chưa đạt cần được xem như lỗi eval/behavior cụ thể chứ không chỉ là nhiễu.
...
Điểm mạnh nổi bật nhất của hệ thống là kiến trúc phòng thủ nhiều tầng. Hai lớp guardrail kẹp ở đầu vào và đầu ra, một system prompt nêu rõ chín quy tắc ứng xử, và trên hết là triết lý suy giảm duyên dáng ở các lớp phụ trợ: khi reranker hỏng thì hệ thống trả về tập ứng viên thô, khi viết lại câu hỏi thất bại thì dùng lại câu gốc, khi truy xuất gặp sự cố thì vẫn có thể gọi LLM với ngữ cảnh hồ sơ và không có tài liệu KB, và khi tóm tắt lỗi thì giữ nguyên bản tóm tắt cũ. Nhờ cách thiết kế ấy, nhiều lỗi phụ trợ không kéo sập toàn bộ lượt chat, một phẩm chất đặc biệt quan trọng với sản phẩm tài chính cần độ tin cậy cao. Lỗi ở LLM chính vẫn được xử lý như sự cố dịch vụ và trả thông báo an toàn qua HTTP 503.
...
Về mặt nội dung, kho tri thức hiện còn nhỏ, mới chỉ gồm `policy.md`, `faq.md` và từ điển dữ liệu, nên chưa phủ hết các tình huống nghiệp vụ phức tạp; chất lượng câu trả lời vì thế bị chặn trên bởi chính độ phủ của kho. Tương tự, việc phát hiện ý định điều chỉnh khoản vay vẫn dựa vào một danh sách cụm từ tiếng Việt phải bảo trì thủ công, khó tổng quát hóa cho những cách hỏi ngoài dự liệu. Hệ thống cũng phụ thuộc vào nhà cung cấp bên ngoài khi cả LLM lẫn embedding đều đi qua OpenRouter, kéo theo rủi ro về độ trễ, chi phí và tính sẵn sàng của bên thứ ba.

Về mặt vận hành, có bốn điểm cần xử lý trước khi coi pipeline là chặt. Thứ nhất, ingest mặc định chưa idempotent vì chưa truyền point ID ổn định cho Qdrant; chạy lại nhiều lần có thể tạo bản sao chunk nếu không `--recreate`. Thứ hai, marker trong prompt cho Loan Adjustment chưa thống nhất với tên khối context mà code thực sự đưa vào. Thứ ba, một số singleton helper chưa có lock khởi tạo, có thể gây khởi tạo lặp trong tải đồng thời cao. Thứ tư, nhánh bắt lỗi tổng quát ở `chat_service` cần tránh đưa nội dung exception ra người dùng; thông báo lỗi nên cố định và chi tiết chỉ ghi log server-side.

Cuối cùng, ngay bản thân thước đo đánh giá cũng còn thiên lệch: chỉ số Context Precision vốn đo độ sạch của tài liệu KB trả về nên không phù hợp với nhóm câu trả lời cá nhân hóa, vốn lấy cơ sở từ User Context chứ không từ KB, như đã phân tích ở mục 5.8.1. Khung cũng chưa đo được Context Recall một cách trực tiếp do thiếu tập "đoạn vàng" tham chiếu.
...
Trên trục tác nhân, bộ công cụ có thể được mở rộng vượt khỏi việc điều chỉnh khoản vay, thêm các công cụ tra cứu trạng thái đơn, ước tính lịch trả nợ, hay mô phỏng kịch bản "nếu-thì" cho điểm tín dụng. Riêng về đánh giá, hai cải tiến đáng ưu tiên là tinh chỉnh khung đo bằng cách tách một chỉ số "user-context grounding" riêng cho nhóm cá nhân hóa thay vì ép chúng vào thước đo precision của KB, đồng thời bổ sung một tập "đoạn vàng" để đo Context Recall đúng nghĩa; song song đó nên chuẩn hóa khớp số đa định dạng và có thể thêm một tầng chấm điểm bằng LLM-as-judge chạy cạnh khung xác định hiện tại để bắt những lỗi ngữ nghĩa mà phép khớp cụm từ bỏ sót. Sau cùng, trên phương diện vận hành, cần bổ sung stable point IDs cho ingest, thống nhất marker Loan Adjustment, harden các singleton helper, và đưa các bộ đếm sẵn có như `get_rerank_stats` cùng tỷ lệ rơi vào nhánh dự phòng lên dashboard quản trị, giúp phát hiện sớm những suy giảm âm thầm của pipeline trước khi chúng ảnh hưởng tới người dùng.
