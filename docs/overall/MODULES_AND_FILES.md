...
- **Cách hoạt động:** Đọc `bronze.home_credit_raw`, chuẩn hóa loan, income, demographic và employment fields.

### transform_gold_homecredit.sql

- **Chức năng:** Transform Silver → Gold cho Home Credit.
- **Mục đích:** Tạo bảng `gold.hc_features_v2` cho `customer_risk_model.pkl` và `scorecard_model.pkl`.
- **Cách hoạt động:** Join DPD, bureau aggregates, CB queries, prolongations và feature engineering cho training (Home Credit 2024 Stability dataset).

## Thư mục docs/

### overall/

- **Chức năng:** Tài liệu tổng quan dự án, kiến trúc app, module/file map và admin guide.
- **Mục đích:** Giúp người đọc nắm bối cảnh hệ thống end-to-end trước khi đi vào ML hoặc RAG.
- **File chính:** `overall.md`, `PROJECT_OVERVIEW.md`, `MODULES_AND_FILES.md`, `ADMIN_GUIDE.md`.

### ml/

- **Chức năng:** Tài liệu dataset, feature, model, scorecard và credit scoring.
- **Mục đích:** Gom toàn bộ nội dung phục vụ ETL/ML training, model contract và credit scoring.
- **File chính:** `ML_FEATURES.md`, `SCORECARD_FEATURES.md`, `2026-05-12-credit-score.md`.
...
- **Cách hoạt động:** Đọc `gold.hc_features_v2`, train pipeline LightGBM 35 features (DPD, bureau, CB queries), lưu metadata contract (`feature_cols`, defaults, thresholds, model_version).
...
- **Cách hoạt động:** Đọc `gold.hc_features_v2`, train scorecard, lưu FICO params và contribution table.
